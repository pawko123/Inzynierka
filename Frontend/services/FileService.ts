...
export class FileService {
	static async downloadAndOpenFile(options: FileDownloadOptions): Promise<void> {
		...
		try {
			...
			Alert.alert(strings.FileDownload.Download_Error, strings.FileDownload.Download_Failed, [
				{ text: strings.FileDownload.OK },
			]);
		}
	}

	static async downloadFileToMobile(
		fileUrl: string,
		token: string,
		attachment: { fileName: string; fileType: string },
	): Promise<void> {
		try {
	...
	static async handleNonMediaFile(
		fileUri: string,
		attachment: { fileName: string; fileType: string },
	): Promise<void> {
		try {
			...
			if (!permissions.granted) {
				Alert.alert(
					...
					strings.FileDownload.Permission_Required,
				);
				return;
			}

    ...
    try {
      ...
      if (serverId) {
        member = await memberRepo.findOne({
          ...
          relations: ["roles", "roles.role", "roles.role.channelPermissions", "roles.role.permissions","roles.role.channelPermissions.channel"],
        });
      ...
      if (channelId && memberRoles.length > 0) {
        ...
        for (const mr of memberRoles) {
          ...
          const channelPermsForRole = role.channelPermissions.filter(cp => cp.channel.id === channelId);
      ...
      if (anyRoleHasChannelPermDefined) {
        ...
        if (!hasChannelPermission) {
          return res.status(403).json({ error: `User does not have channel permission: ${permission}` });
        }
      } else {
        ...
        if (!hasServerPermission) {
          return res.status(403).json({ error: `User does not have server permission: ${permission}` });
        }
      }

...
const createChannel = async (req: Request, res: Response) => {
  try {
    ...
    if (type && !Object.values(ChannelType).includes(type)) {
      return res.status(400).json({ message: `Invalid channel type. Must be one of: ${Object.values(ChannelType).join(", ")}` });
    }
...
const createDirectChannel = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: "Error creating direct channel", error: err.message });
  }
};
...
const addMemberToDirectChannel = async (req: Request, res: Response) => {
  try {
    ...
    if (!channelId || !invitedUserId || !userId) {
      return res.status(400).json({ message: "channelId, userId, and invitedUserId are required" });
    }
    const channelRepo = AppDataSource.getRepository(Channel);
    ...
    const inviterParticipant = await participantRepo.findOne({ where: { channel: { id: channel.id }, user: { id: String(userId) } } });
    if (!inviterParticipant) {
      return res.status(403).json({ message: "Inviter must be a participant of the channel" });
    }
    ...
    const existing = await participantRepo.findOne({ where: { channel: { id: channel.id }, user: { id: user.id } } });
    if (existing) {
    ...
    return res.status(500).json({ message: "Error adding member to direct channel", error: err.message });
  }
};
...
const getDirectChannelParticipants = async (req: Request, res: Response) => {
  try {
    ...
    const isParticipant = await participantRepo.findOne({ where: { channel: { id: channel.id }, user: { id: String(userId) } }, relations: ["user"] });
    if (!isParticipant) {
    ...
    return res.status(500).json({ message: "Error getting channel participants", error: err.message });
  }
};

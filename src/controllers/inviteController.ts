...
export const acceptInvite = async (req: Request, res: Response) => {
    ...
    try {
        await AppDataSource.transaction(async (manager) => {
            ...
            const existingMember = await memberRepo.findOne({ where: { user: { id: userId }, server: { id: invite.server.id } }, relations: ['user', 'server'] });
            if (existingMember) {
        ...
        return res.status(200).json({ message: 'Joined server successfully.', serverId: (req as any)._joinedServerId });
    } catch (err) {
